        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={`bg-white rounded-xl border ${isFeatured ? 'border-brand-gold' : 'border-surface-border'} shadow-sm p-6`}>
            <div className="flex items-start gap-5">
              {listing.photo_url ? (<img src={listing.photo_url} alt={listing.full_name} className="w-20 h-20 rounded-full object-cover border-2 border-brand-primary/20 shrink-0"/>) : (<div className="w-20 h-20 rounded-full bg-brand-primary/10 flex items-center justify-center shrink-0">
              ...
              <div className="flex-1">
                ...
                {listing.practice_name && (<p className="text-gray-600 font-medium">{listing.practice_name}</p>)}
                ...
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1.5 text-sm text-gray-600">
                  ...
                  {listing.license_number && (<span className="flex items-center gap-1.5 text-brand-primary">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {null}
                  {listing.is_aanp_member && (<span className="flex items-center gap-1 text-xs bg-brand-gold/10 text-brand-gold font-medium px-2.5 py-1 rounded-full">
                      <Star className="w-3.5 h-3.5" aria-label="AANP"/>
                      AANP Member
                    </span>)}
                  {listing.offers_telemedicine && (<span className="flex items-center gap-1 text-xs bg-blue-50 text-blue-700 font-medium px-2.5 py-1 rounded-full">
                      <Wifi className="w-3.5 h-3.5" aria-label="Telemedicine"/>
                      Telemedicine
                    </span>)}
                  {listing.accepting_new_patients && (<span className="flex items-center gap-1 text-xs bg-green-50 text-green-700 font-medium px-2.5 py-1 rounded-full">
          ...
          {/* Bio */}
          {listing.bio && (<div className="bg-white rounded-xl border border-surface-border shadow-sm p-6">
          ...
          {/* Specialties */}
          {listing.specialties.length > 0 && (<div className="bg-white rounded-xl border border-surface-border shadow-sm p-6">
              <h2 className="font-bold text-gray-900 mb-4">Specialties</h2>
              <div className="flex flex-wrap gap-2">
                {listing.specialties.map((s) => (<Link key={s} href={`/listings?specialty=${s}`} className="text-sm bg-brand-primary/5 border border-brand-primary/15 text-brand-primary-dark hover:bg-brand-primary/10 px-3 py-1.5 rounded-full transition-colors">
                    {SPECIALTIES[s] ?? s}
                  </Link>))}
              </div>
            </div>)}
          ...
          {listing.treatment_modalities.length > 0 && (<div className="bg-white rounded-xl border border-surface-border shadow-sm p-6">
              <h2 className="font-bold text-gray-900 mb-4">Treatment Modalities</h2>
              <div className="flex flex-wrap gap-2">
                {listing.treatment_modalities.map((m) => (<span key={m} className="text-sm bg-surface border border-surface-border text-gray-700 px-3 py-1.5 rounded-full">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-surface-border shadow-sm p-5 sticky top-6">
            ...
            {(<div className="space-y-3">
                {listing.phone && (<a href={`tel:${listing.phone}`} className="flex items-center gap-3 p-3 bg-surface rounded-lg hover:bg-brand-primary/5 transition-colors group">
                ...
                {listing.website && (<a href={listing.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-surface rounded-lg hover:bg-brand-primary/5 transition-colors group">
                ...
                {listing.email && (<a href={`mailto:${listing.email}`} className="flex items-center gap-3 p-3 bg-surface rounded-lg hover:bg-brand-primary/5 transition-colors group">
                ...
                {listing.accepts_insurance && (<div className="mt-4 flex items-center gap-2 text-sm text-green-700 bg-green-50 rounded-lg px-3 py-2">
                    <CheckCircle className="w-4 h-4 shrink-0" aria-label="Insurance"/>
                    Accepts Insurance
                  </div>)}
              </div>)}

            {!isClaimed && (<div className="mt-4 pt-4 border-t border-surface-border">
                <p className="text-xs text-gray-500 mb-2">Is this your practice?</p>
                <Link href={`/claim/${listing.id}`} className="block w-full text-center border border-brand-primary text-brand-primary hover:bg-brand-primary hover:text-white font-medium text-sm py-2.5 rounded-lg transition-colors">
                  Claim This Profile
                </Link>
              </div>)}
          </div>
          ...
          <div className="bg-white rounded-xl border border-surface-border shadow-sm p-5">
            ...
            <dl className="space-y-2 text-sm">
              {listing.license_state && (<div className="flex justify-between">
