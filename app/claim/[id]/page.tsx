...
export default function ClaimPage({ params, searchParams }: PageProps) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('directory_slug', 'naturopathic-doctors')
      .eq('listing_id', id)
    ...
    return (
      <div className="min-h-64 flex items-center justify-center py-20">
        <div className="text-center">
          <Loader2 className="w-10 h-10 text-brand-primary animate-spin mx-auto mb-4" aria-label="Loading" />
  ...
  if (verifyState === 'error') {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <Link href={`/claim/${id}`} className="bg-brand-primary text-white font-semibold px-6 py-3 rounded-lg hover:bg-brand-primary-dark transition-colors">
          Request New Link
        </Link>
      </div>
    )
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        <div className="bg-white rounded-xl border border-surface-border shadow-sm p-8 text-center mb-6">
          ...
          <p className="text-gray-600">
            Your profile is now claimed. Upgrade to start receiving inquiries from patients.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-white rounded-xl border border-brand-primary shadow-sm p-6">
            <div className="flex items-center gap-2 mb-3">
              <CheckCircle className="w-5 h-5 text-brand-primary" aria-label="Verified" />
              <h2 className="font-bold text-gray-900">Verified ND</h2>
              <span className="ml-auto text-brand-primary font-bold">$99/yr</span>
            </div>
            <ul className="text-sm text-gray-700 space-y-2 mb-5">
              {['Photo & professional bio', 'All specialty tags', 'Contact form from patients', 'Licensed ND badge', 'Priority over free listings'].map((f) => (
            ...
            <button
              ...
              className="w-full bg-brand-primary hover:bg-brand-primary-dark text-white font-semibold py-2.5 rounded-lg transition-colors"
            >
              Upgrade to Verified
            </button>
          </div>
          ...
          <div className="bg-white rounded-xl border border-brand-gold shadow-sm p-6 relative">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-bold text-gray-900">Featured ND</h2>
              <span className="ml-auto text-brand-gold font-bold">$199/yr</span>
            </div>
            <ul className="text-sm text-gray-700 space-y-2 mb-5">
            ...
            <button
              ...
              className="w-full bg-brand-gold hover:bg-brand-gold-light text-white font-semibold py-2.5 rounded-lg transition-colors"
            >
              Upgrade to Featured
            </button>
          </div>
  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-600">
          We sent a verification link to <strong>{email}</strong>. Click it to claim your listing and access your upgrade options.
        </p>
      </div>
    )
  }
    ...
    <div className="max-w-lg mx-auto px-4 py-12">
      <div className="text-center mb-8">
        <div className="inline-flex items-center justify-center w-14 h-14 bg-brand-primary/10 rounded-full mb-4">
          <Shield className="w-7 h-7 text-brand-primary" aria-label="Claim" />
        </div>
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Claim Your Listing</h1>
        <p className="text-gray-600">
          Enter your professional email address to receive a verification link. We&apos;ll confirm you&apos;re the licensed ND in this listing.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-surface-border shadow-sm p-6">
        <form onSubmit={handleClaim} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-brand-primary hover:bg-brand-primary-dark text-white font-semibold py-3 rounded-lg transition-colors disabled:opacity-60"
          >
        ...
        <p className="text-xs text-gray-500 mt-4 text-center">
          We&apos;ll send a one-time link to verify you are the licensed ND named in this listing.
        </p>
      </div>
    </div>
  )
