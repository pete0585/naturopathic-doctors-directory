...
export async function POST(req: NextRequest) {
  ...
  if (!listingId || !tier) {
    return NextResponse.json({ error: 'Missing listingId or tier' }, { status: 400 })
  }
  ...
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://naturopathicdoctorfinder.com'
